...
class MonthlyReport extends Model
{
    ...
    /**
     * Get the year that owns the monthly report
     */
    ...
    /**
     * Get the month that owns the monthly report
     */
    ...
    /**
     * Boot method to automatically set report_date
     */
    protected static function boot()
    {
        parent::boot();

        static::creating(function ($monthlyReport) {
            if (!$monthlyReport->report_date) {
                $monthlyReport->report_date = $monthlyReport->generateReportDate();
            }
            
            if (!$monthlyReport->title) {
                $monthlyReport->title = $monthlyReport->generateDefaultTitle();
            }
        });

        static::updating(function ($monthlyReport) {
            if ($monthlyReport->isDirty(['year_id', 'month_id'])) {
                $monthlyReport->report_date = $monthlyReport->generateReportDate();
            }
        });
    }
    ...
    /**
     * Generate the report date (last day of the month)
     */
    public function generateReportDate(): string
    {
        if ($this->year && $this->month) {
            return $this->month->getLastDayOfMonth($this->year->year_value);
        }
        ...
        if ($year && $month) {
            return $month->getLastDayOfMonth($year->year_value);
        }
    ...
    public function generateDefaultTitle(): string
    {
        ...
        $month = $this->month ?? Month::find($this->month_id);
        
        if ($year && $month) {
            return "Monthly Water Quality Report - {$month->month} {$year->year_value}";
        }

        return "Monthly Water Quality Report";
    }
    ...
    {
        return "{$this->month->month} {$this->year->year_value}";
    }
